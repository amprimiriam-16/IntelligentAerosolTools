...
#
# Last modification March 14, 2020 (by AZ)
# Run this script with chosen input arguments as:  
#     python .\PureCompProperties_for_LumpingSchemes.py  ./inputfiles/smiles_1409.txt  300.0

import glob
#AZ: changed from original code: 'import pybel'
...
#sys.path.append('C:/Users/damala/Documents/AIOMFAC_folder/AIOMFAC-Program/AIOMFAC-web_2.20new')
sys.path.append('./umansysprop')
...
#print("Enter the temperature of the system for vapor pressure calculation in Kelvin : ")
#temperature = float(temperature1)

#AZ: read the command line argument 1 as directory\filename (of smiles input file)
inpfile = sys.argv[1]     
inpfile = str(inpfile)
#AZ: temperature in [K] as second (input) command line argument
temperature1 = sys.argv[2]  
temperature = float(temperature1)

#use input file name and relative path to generate output file name and relative path:
print('SMILES input file is: ', inpfile)
outpfile = inpfile.replace('smiles', 'vaporpressure')
outpfile = outpfile.replace('InputFiles', 'OutputFiles')
#print('outpfile is: ', outpfile)
...
#AZ: following code line changed to remove depreciated 'U' mode
text = open(inpfile[:],mode='r',newline=None)  
...
for line in text.readlines():
	if line != '\n':
		...
		smiles_array.append(input[0])
		# Now create Pybel objects which are used in all property predictive
		# techniques
		Pybel_object = pybel.readstring('smi',input[0])
		Pybel_object_dict[input[0]] = Pybel_object       
##########################################################################################       
...
for smiles in smiles_array:
    ...
    vapour_pressure_dict[smiles]['EVAP'],vapour_pressure_dict[smiles]['EVAPA'],vapour_pressure_dict[smiles]['EVAPB']=vapour_pressures.evaporation(Pybel_object_dict[smiles], temperature)
...
file_name = open(outpfile,'w')
file_name.write(str('CpNo')+'\t'+str('SMILES')+'\t'+'\t'+'\t'+str('log_10(VP_Nannoolal)')+'\t'+str('log_10(EVAP)')+'\t'+str('EVAPParamA')+'\t'+str('EVAPParamB')) 
    
for index,smiles in enumerate(smiles_array):

    file_name.write('\n')
    file_name.write(str(index+2)+'\t'+'\t'+str(smiles)+'\t'+'\t'+'\t'+str(vapour_pressure_dict_BP[smiles]['VP_Nannoolal']['BP_Nannoolal'])+'\t'+str(vapour_pressure_dict[smiles]['EVAP'])+'\t'+str(vapour_pressure_dict[smiles]['EVAPA'])+'\t'+str(vapour_pressure_dict[smiles]['EVAPB']))  

file_name.write('\n')
file_name.write('----')
file_name.close()
print('The output is in: ', outpfile)
